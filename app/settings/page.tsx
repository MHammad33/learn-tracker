...
export default function Settings() {
  ...
  const exportData = () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i)
      if (key && (key.startsWith('learning-entry-') || key === 'learning-streak')) {
    ...
    a.href = url
    a.download = `learn-tracker-backup-${new Date().toISOString().split('T')[0]}.json`
  ...
  const clearAllData = () => {
    if (confirm('Are you sure you want to delete all your learning entries? This action cannot be undone.')) {
        ...
        <Card className="lg:col-span-2 relative overflow-hidden border-0 bg-gradient-to-br from-white to-orange-50/50 dark:from-gray-900 dark:to-orange-950/20 shadow-xl hover:shadow-2xl transition-all duration-300">
          ...
          <CardContent className="space-y-4 relative">
            ...
            <div className="text-xs text-muted-foreground space-y-1">
              ...
              <p>• Clear: Permanently delete all learning entries and reset streak</p>
            </div>
          </CardContent>
        </Card>
      </div>
